...
class SelfRAG:
    ...
    def filter_relevant_passages(
        ...
        # Fast path: if passages already have rerank_scores, use them
        reranked = [p for p in passages if p.get("rerank_score") is not None]
        if len(reranked) == len(passages):
            # Normalize cross-encoder score (roughly -10..10) to 0-1
            result = []
            for p in passages:
                raw = float(p["rerank_score"])
                norm = max(0.0, min(1.0, (raw + 5.0) / 10.0))
                if norm >= threshold:
                    d = dict(p)
                    d["relevance_score"] = round(norm, 4)
                    d["reflection_token"] = (
                        REFLECTION_TOKENS["relevant"]
                        if norm >= 0.5
                        else REFLECTION_TOKENS["irrelevant"]
                    )
                    result.append(d)
            return result if result else list(passages)  # never return empty
